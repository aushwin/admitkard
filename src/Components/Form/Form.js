...
const Form = () => {
  ...
  return (
    <form className={Styles.wrapper} onSubmit={onFormSubmit} ref={formRef}>
      ...
      />
      {!nameValid && nameValid !== "" && (
        <p className={Styles.errorMessage}>minimum 3 character</p>
      )}
      <input
      ...
      {!emailValid && emailValid !== "" && (
        <p className={Styles.errorMessage}>email is not valid</p>
      )}
      <input
        style={styleError(numberValid)}
        type="tel"
      ...
      {!numberValid && numberValid !== "" && (
        <p className={Styles.errorMessage}>
          not valid character or number less than 10 digits
        </p>
      )}

      <select
        style={styleError(courseValid)}
        name="course"
        id="course"
      ...
      <div
        className={`${Styles.alert} ${
          alert === "updated" && Styles.alertUpdated
        } ${alert === "new" && Styles.alertSuccess}`}
      >
        {loading && <Spinner />}
